...
def parse_args():
    ...
    p.add_argument("--evof-ckpt",   default=None,       help="EvoFormer/OpenFold checkpoint path")
    p.add_argument("--source-pdb",  default=None,       help="Source bacterial NRPS PDB file")
    p.add_argument("--msa-file",    default=None,       help="Animal NRPS MSA file (.a3m or .fasta)")
    p.add_argument("--output-dir",  default="results/", help="Output directory")
    p.add_argument("--device",      default="cuda" if torch.cuda.is_available() else "cpu")
    p.add_argument("--flow-steps",  type=int, default=20, help="OT-Flow Matching NFE (default 20)")
    p.add_argument("--no-rag",      action="store_true",  help="Disable structural retrieval RAG")
    return p.parse_args()
...
def main():
    ...
    with open(fasta_path, "w") as f:
        for i, seq in enumerate(results['pareto_sequences']):
            ...
            f.write(f">design_{i:04d} | evol={scores[0]:.3f} stab={scores[1]:.3f} "
