...
class MSARowAttentionWithPairBias(nn.Module):
    ...
    def __init__(self, c_msa: int = C_MSA, c_z: int = C_Z,
                 n_head: int = N_HEAD_MSA, c_head: int = 32):
        super().__init__()
        self.n_head = n_head
        self.c_head = c_head
...
class MSAColumnAttention(nn.Module):
    ...
    """
    def __init__(self, c_msa: int = C_MSA, n_head: int = N_HEAD_MSA,
                 c_head: int = 32):
        super().__init__()
        self.n_head = n_head
        self.c_head = c_head
...
class OuterProductMean(nn.Module):
    ...
    def forward(self, msa: torch.Tensor) -> torch.Tensor:
        ...
        outer = torch.einsum('bsid,bsje->bijd', a, 
                             self.proj2(msa_n).permute(0, 2, 1, 3)) / N_seq
...
class TriangularMultiplicativeUpdate(nn.Module):
    ...
    """
    def __init__(self, c_z: int = C_Z, c_hidden: int = 128,
                 outgoing: bool = True):
        super().__init__()
...
class TriangularSelfAttention(nn.Module):
    ...
    """
    def __init__(self, c_z: int = C_Z, n_head: int = N_HEAD_PAIR,
                 c_head: int = 32, row_wise: bool = True):
        super().__init__()
        self.n_head   = n_head
        self.c_head   = c_head
    ...
    def forward(self, pair: torch.Tensor) -> torch.Tensor:
        ...
        scale = math.sqrt(self.c_head)
        attn  = F.softmax(
            torch.matmul(Q, K.transpose(-1,-2)) / scale + bias, dim=-1
        )
...
class EvoFormer(nn.Module):
    ...
    """
    def __init__(self, n_blocks: int = 48, vocab_size: int = 22,
                 c_msa: int = C_MSA, c_z: int = C_Z, c_s: int = C_S,
                 max_len: int = 1024):
        super().__init__()
        # Input embeddings
    ...
    def _build_pair_input(self, L: int, device: torch.device,
