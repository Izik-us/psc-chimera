...
def random_so3_frames(batch: int, L: int,
...
class InvariantPointAttention(nn.Module):
    ...
    """
    def __init__(self, c_s: int = 256, c_z: int = 256, n_head: int = 12,
                 n_qk_pts: int = 4, n_v_pts: int = 8, c_hidden: int = 16):
        super().__init__()
        self.n_head    = n_head
    ...
    def _apply_frames(self, pts: torch.Tensor,
    ...
    def forward(self, s: torch.Tensor, z: torch.Tensor,
        ...
        o_pts_local = torch.einsum('blij,blhpj->blhpi',
                                   R_inv, o_pts - t.unsqueeze(-2).unsqueeze(-2))
...
class SE3Block(nn.Module):
    ...
    def forward(self, s: torch.Tensor, z: torch.Tensor,
        ...
        R_new    = torch.bmm(
            R.view(-1, 3, 3), R_update.view(-1, 3, 3)
        ).view(*R.shape)
...
class SE3DiffusionSchedule(nn.Module):
    ...
    def add_noise(self, t_coords: torch.Tensor, R_frames: torch.Tensor,
        ...
        R_noisy     = torch.bmm(
            R_frames.view(-1, 3, 3), R_noise.view(-1, 3, 3)
        ).view(*R_frames.shape)
...
class SE3Denoiser(nn.Module):
    ...
    """
    def __init__(self, c_s: int = 256, c_z: int = 256, n_blocks: int = 8,
                 max_t: int = 200):
        super().__init__()
    ...
    def forward(self, R_noisy: torch.Tensor,
        ...
        node_features = torch.cat([
            t_noisy.mean(-1, keepdim=True).expand(-1, -1, 2),  # rough position encoding
    ...
    @torch.no_grad()
    def sample(self, L: int, pair_repr: torch.Tensor,
               motif_mask: Optional[torch.Tensor] = None,
               motif_coords: Optional[torch.Tensor] = None,
        ...
        for step in reversed(range(n_steps)):
            ...
            t = (1 / alpha.sqrt()) * (t - beta / (1 - abar).sqrt() * (t - abar.sqrt() * t_pred)) \
                + beta.sqrt() * noise

            # Denoise step for rotation (simplified: blend toward predicted)
            blend = 1 - alpha
            R_update = axis_angle_to_matrix(
                matrix_to_axis_angle(R_pred) * blend
            )
