...
class BackboneEncoder(ABC):
    ...
    @abstractmethod
    def forward(self, msa_tokens: torch.Tensor, pair_features: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
class StructureGenerator(ABC):
    ...
    @abstractmethod
    def sample(self, source_R: torch.Tensor, source_t: torch.Tensor, conditioning: Any, steps: int) -> tuple[torch.Tensor, torch.Tensor]:
        raise NotImplementedError
...
class SequenceDesigner(ABC):
    ...
    def design(
        ...
        fixed_aas: torch.Tensor | None = None,
...
class OpenFoldCLIAdapter(BackboneEncoder):
    ...
    def __init__(self, checkpoint: str | Path, runner_script: str | Path, python_executable: str = "python", timeout_seconds: int = 3600) -> None:
        self.checkpoint = _require_file(checkpoint, "OpenFold")
    ...
    def forward(self, msa_tokens: torch.Tensor, pair_features: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with tempfile.TemporaryDirectory(prefix="chimera-openfold-") as workdir:
            ...
            torch.save({"msa_tokens": msa_tokens.cpu(), "pair_features": pair_features.cpu()}, input_path)
            command = [self.python_executable, str(self.runner_script), "--checkpoint", str(self.checkpoint), "--input", str(input_path), "--output", str(output_path)]
        ...
        if single_repr.shape[:2] != (msa_tokens.shape[0], msa_tokens.shape[2]) or pair_repr.shape[:3] != (msa_tokens.shape[0], msa_tokens.shape[2], msa_tokens.shape[2]):
...
class RFdiffusionAdapter(StructureGenerator):
    ...
    def __init__(self, checkpoint: str | Path, **_: Any) -> None:
        ...
        raise BackendUnavailable(
            "RFdiffusionAdapter is not wired to RFdiffusion inference in this prototype; use RFdiffusion's native API instead of SE3FlowMatching"
        )
...
class RFdiffusionCLIAdapter(StructureGenerator):
    ...
    def __init__(self, checkpoint: str | Path, inference_script: str | Path, python_executable: str = "python", timeout_seconds: int = 3600) -> None:
        self.checkpoint = _require_file(checkpoint, "RFdiffusion")
    ...
    def sample(self, source_R: torch.Tensor, source_t: torch.Tensor, conditioning: Any, steps: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not isinstance(conditioning, Mapping):
            raise ValueError("RFdiffusionCLIAdapter conditioning must be a mapping with input_pdb, contigs, and output_prefix")
        ...
        if completed.returncode != 0:
            detail = completed.stderr.strip() or completed.stdout.strip()
            raise BackendUnavailable(f"RFdiffusion inference failed with exit code {completed.returncode}: {detail}")

        generated_pdb = Path(f"{output_prefix}_0.pdb")
        if not generated_pdb.is_file():
            raise BackendUnavailable(f"RFdiffusion completed without producing {generated_pdb}")
...
class ProteinMPNNAdapter(SequenceDesigner):
    ...
    Fixed positions must be accompanied by their actual amino-acid IDs. A mask
    alone cannot preserve residue identity because ProteinMPNN's ``S_true`` is
    also the source of the sequence at non-designed positions.
    """
    ...
    def __init__(self, checkpoint: str | Path, proteinmpnn_repo: str | Path | None = None, device: str | torch.device = "cpu", temperature: float = 0.1) -> None:
        ...
        try:
            ...
            raise BackendUnavailable("ProteinMPNN checkpoint does not match the native adapter architecture") from exc
        self.model.to(self.device).eval()

    def design(self, backbone_coords: torch.Tensor, fixed_positions: torch.Tensor | None = None, fixed_aas: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if fixed_positions is None:
            ...
            fixed_positions = fixed_positions.to(self.device).bool()
            if fixed_positions.shape != (batch_size, length):
                raise ValueError("fixed_positions must have shape (B, L)")
            if fixed_aas is None:
                raise ValueError("fixed_aas is required when fixed_positions is provided")
            fixed_aas = fixed_aas.to(self.device, dtype=torch.long)
            if fixed_aas.shape != (batch_size, length):
                raise ValueError("fixed_aas must have shape (B, L)")
            if torch.any(fixed_positions & ((fixed_aas < 0) | (fixed_aas >= 21))):
                raise ValueError("fixed_aas contains invalid ProteinMPNN token IDs at fixed positions")
            design_mask = mask * (~fixed_positions).float()
            sequence_seed = fixed_aas.clone()
        ...
        if fixed_positions is not None and not torch.equal(sequences[fixed_positions], fixed_aas[fixed_positions]):
            raise RuntimeError("native ProteinMPNN violated a fixed-residue identity constraint")
