...
def merge_ready_so3_log(R: torch.Tensor) -> torch.Tensor:
    """Numerically stable principal SO(3) logarithm.

    The legacy diagonal-axis fallback loses the signs of the rotation axis near
    pi. A branch-selected quaternion representation retains those signs. The
    logarithm remains necessarily non-unique at the pi cut locus, but this
    implementation is stable on either side of it.
    """
    if R.shape[-2:] != (3, 3):
    ...
    # Quaternion vector part, selecting the largest stable diagonal branch.
    d0 = (1.0 + R[..., 0, 0] - R[..., 1, 1] - R[..., 2, 2]).clamp_min(0.0)
    d1 = (1.0 - R[..., 0, 0] + R[..., 1, 1] - R[..., 2, 2]).clamp_min(0.0)
    d2 = (1.0 - R[..., 0, 0] - R[..., 1, 1] + R[..., 2, 2]).clamp_min(0.0)
    qx = 0.5 * torch.sqrt(d0)
    qy = 0.5 * torch.sqrt(d1)
    qz = 0.5 * torch.sqrt(d2)
    qx = qx * torch.where(R[..., 2, 1] - R[..., 1, 2] >= 0, 1.0, -1.0)
    qy = qy * torch.where(R[..., 0, 2] - R[..., 2, 0] >= 0, 1.0, -1.0)
    qz = qz * torch.where(R[..., 1, 0] - R[..., 0, 1] >= 0, 1.0, -1.0)
    q = torch.stack([qx, qy, qz], dim=-1)

    # For moderate angles, use the skew formula because it is smoother.
    sin_half = q.norm(dim=-1)
    scale = 2.0 * theta / sin_half.clamp_min(1e-7)
    quat_omega = q * scale.unsqueeze(-1)
    small = theta < 1e-4
    result = torch.where(small.unsqueeze(-1), vee, quat_omega)
    return result
