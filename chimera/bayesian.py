...
def _dropout_only(model: nn.Module):
    """Enable Dropout layers while preserving BatchNorm/eval state."""
...
class BayesianUncertaintyEstimator:
    ...
    def predict(
        ...
        """Return predictive mean, epistemic variance/std and optional aleatoric variance.

        ``output_getter`` maps a model output to a tensor representing the
        scalar objective. If the model returns a dict with ``mean`` and
        ``variance`` keys, those are used automatically.
        """
        ...
        if predictive_variances:
            result["aleatoric_variance"] = torch.stack(predictive_variances, dim=0).mean(dim=0).clamp_min(0)
            result["total_variance"] = variance + result["aleatoric_variance"]
        return result
    ...
    def expected_improvement(
        ...
        """Analytic Gaussian EI. ``std`` is a standard deviation, never a variance."""
        ...
        # Deterministic limit as sigma -> 0.
    ...
    def scalarize_objectives(
        ...
        if lower is None or upper is None:
            # Robust per-objective normalization across the candidate batch.
